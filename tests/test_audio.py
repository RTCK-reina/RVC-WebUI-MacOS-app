...
# conftest.py が numpy/av を MagicMock でスタブするため、importorskip だけでは
# 「本物の numpy が存在する」ことを保証できない。isinstance チェックで区別する。
# ---------------------------------------------------------------------------
from unittest.mock import MagicMock as _MagicMock

try:
    import numpy as _np_real
    import av as _av_real
    if isinstance(_np_real, _MagicMock) or isinstance(_av_real, _MagicMock):
        pytest.skip(
            "real numpy/av not installed (conftest stubs detected)",
            allow_module_level=True,
        )
except ImportError as _e:
    pytest.skip(f"numpy or av not available: {_e}", allow_module_level=True)

import numpy as np  # noqa: E402 — confirmed real above
import av as _av    # noqa: F401, E402
