...
class TestCancelEventPropagation:
    def test_cancel_event_stops_iteration(self):
        ...
        # 外部スレッドのタイミングに依存しない決定論的テスト:
        # ループ自身が STOP_AFTER 件処理後に cancel_event を set する。
        # これは rpc_server の訓練ループが cancel_event.is_set() をチェックするパターンと同じ。
        STOP_AFTER = 5
        ...
        def batch_loop(items):
            for i in items:
                ...
                processed.append(i)
                if len(processed) == STOP_AFTER:
                    cancel_event.set()  # 自スレッドからセット → 競合なし
            return {"status": "success"}

        result = batch_loop(range(1000))

        assert result["status"] == "cancelled"
        assert len(processed) == STOP_AFTER
