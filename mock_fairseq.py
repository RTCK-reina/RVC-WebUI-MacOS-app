...
def load_model_ensemble_and_task(
    filenames, arg_overrides=None, task=None, suffix="", **kwargs
):
    """Mock function to replace fairseq checkpoint_utils.load_model_ensemble_and_task"""
    warnings.warn(
        "Using mock fairseq implementation - some features may not work correctly"
    )
    ...
    for filename in filenames:
        try:
            ...
            model = torch.nn.Module()
            model.extract_features = lambda source, **kwargs: torch.zeros(
                1, source.size(1), 768
            )
            models.append(model)
...
def load_checkpoint_to_cpu(path, arg_overrides=None, load_on_all_ranks=False):
    ...
    warnings.warn(
        "Using mock fairseq implementation - loading PyTorch checkpoint directly"
    )
